    ...
    def print_regs(self, outfile, ramdump) :
        for reg_name, t32_name, print_pc in tzbsp_register_names :
            if print_pc :
                ...
                if a is not None :
                    symname, offset = ramdump.unwind_lookup(self.regs[reg_name])
                    pc_string = "[{0}+0x{1:x}]".format(symname,offset)
                else :
                    pc_string = ""
            else :
                pc_string = ""
            print_out_str ("   {0} = 0x{1:08x} {2}".format(reg_name,self.regs[reg_name], pc_string))
...
class TZRegDump(RamParser) :
    ...
    def dump_all_regs(self, out_path) :
        if out_path is None :
            out_path = ""
        for i in range(0,self.ncores) :
            coren_regs = open(out_path+"/core{0}_regs.cmm".format(i),"wb")

            if (self.sc_status[i] & tz_sc_ignore_status) == 0:
                if self.version == 0 :
                    if (self.sc_status[i] & tz_sc_status_ns) :
                        print_out_str ("Core {0} was in the non-secure world".format(i))

                    if (self.sc_status[i] & tz_sc_status_wdt) :
                        print_out_str ("Core {0} experienced a watchdog timeout".format(i))

                    if (self.sc_status[i] & tz_sc_status_sgi) :
                        print_out_str ("Core {0} did not experience a watchdog timeout but some other core did".format(i))
                else :
                    if (self.sc_status[i] & v2tzbsp_sc_status_ns_bit) :
                        print_out_str ("Core {0} was in the non-secure world".format(i))
                    else :
                        print_out_str ("Core {0} was in the secure world".format(i))

                    if (self.sc_status[i] & v2tzbsp_sc_status_sgi) :
                        print_out_str ("Core {0} received an SGI interrupt".format(i))

                    if (self.sc_status[i] & v2tzbsp_sc_status_wdt) :
                        print_out_str ("Core {0} recieved the watchdog interrupt".format(i))

                    if (self.sc_status[i] & v2tzbsp_sc_status_warm_boot) :
                        print_out_str ("core {0} has the warm boot flag set".format(i))

                    print_out_str ("Core {0} WDT status: {1:x}".format(i, self.wdt0_status[i]))

            if (self.version >= 3):
                print_out_str ("status:{0}".format(hex(self.sc_status[i])))
                if (self.sc_status[i] & v3sdi_cpu_dump_status) :
                     print_out_str ("SDI dumped CPU context for core {0}".format(i))
                elif (self.sc_status[i] & v3tzbsp_cpu_dump_status) :
                     print_out_str ("TZ dumped CPU context for core {0}".format(i))

            print_out_str ("core{0} regs:".format(i))
            self.core_regs[i].print_regs(coren_regs, self.ramdump)
            coren_regs.close()

        secure_regs = open(out_path+"/secure_world_regs.cmm","wb")
        print_out_str ("\n=============== secure contex ===========")
    ...
    def dump_core_pc(self, core) :
        ...
        if a is not None :
            ...
            symname = "UNKNOWN"
            offset = 0
        print_out_str ("Core {3} PC: {0}+{1:x} <{2:x}>".format(symname,offset,pc,core))
        ...
        if a is not None :
            ...
            symname = "UNKNOWN"
            offset = 0
        print_out_str ("Core {3} LR: {0}+{1:x} <{2:x}>".format(symname,offset,lr,core))
        print_out_str ("")
        self.ramdump.unwind.unwind_backtrace(self.core_regs[core].regs["svc_r13"], 0, pc, lr, "")
        print_out_str ("")
    ...
    def init_regs(self, ebi_addr) :
        ...
        if status is None :
            ...
            print_out_str ("!!! Do you need to specify a different offset for TZ?")
            return False
        ...
        for i in range(0,cpu_count) :
            self.sc_regs.append(self.ramdump.read_string(ebi_addr, tzbsp_dump_cpu_ctx_t, False))
            ...
            if version > 1 :
                ...
                self.wdog_pc.append(self.ramdump.read_word(ebi_addr + 4, False))
                ebi_addr += 8

        sc_secure = self.ramdump.read_string(ebi_addr, tzbsp_dump_cpu_ctx_t, False)
        ebi_addr += struct.calcsize(tzbsp_dump_cpu_ctx_t)
    ...
    def parse(self) :
        ...
        if ebi_addr is None :
            print_out_str ("!!! Could not read from IMEM at address {0:x}".format(self.ramdump.tz_addr))
            return None

        if (ebi_addr == 0) :
            print_out_str ("!!! No EBI address at IMEM location {0:x}.".format(self.ramdump.tz_addr))
            print_out_str ("!!! No FIQ occured on this system")
            return None

        print_out_str ("[!!!!] Read {0:x} from IMEM successfully!".format(ebi_addr))
        ...
        if self.ramdump.is_config_defined("CONFIG_MSM_DEBUG_IMAGE") :
            print_out_str ("[!!!!] Debug image was enabled, the contexts will be printed there")
            return
...
def get_wdog_timing(ramdump) :
    ...
    timer_expires_offset  = ramdump.field_offset("struct timer_list", "expires")

    timer_expires = ramdump.read_word(dogstruct_addr + timer_offset + timer_expires_offset)
    jiffies = ramdump.read_word(jiffies_addr)
    if (timer_expires > jiffies) :
        print_out_str ("Pet function to be scheduled in {0} seconds".format((timer_expires - jiffies) * 0.01))
    else :
        print_out_str ("Pet function may have been blocked.")
        print_out_str ("Jiffies is {0} seconds after when  it was supposed to be scheduled".format((jiffies - timer_expires) *0.01))
    ...
    if (last_ns_addr != 0) and (last_pet_addr != 0) and (pet_delay_time_addr != 0) :
        ...
        print_out_str ("Most recent time that the watchdog was pet: {0}.{1:6}".format(last_pet/1000000000, last_pet % 1000000000))
        print_out_str ("Most recent timestamp read from the timer hardware: {0}.{1:6}".format(last_ns/1000000000, last_ns % 1000000000))
        if (last_ns > last_pet) :
            print_out_str ("Pet delay time in this build is {0} seconds".format((pet_delay_time * 10)/1000))
            print_out_str ("The watchdog was not pet for at least {0}.{1:6} seconds".format(diff / 1000000000, diff % 1000000000))

        print_out_str ("!!! The difference between the two timestamps above is NOT")
        print_out_str ("!!! to be used to determine whether a watchdog bite occured")

    elif (last_pet_addr) :
        last_pet = ramdump.read_dword(last_pet_addr)
        print_out_str ("Most recent time that the watchdgo was pet: {0}.{1:6}".format(last_pet / 1000000000, last_pet % 1000000000))
        print_out_str ("The watchdog was not pet for at least {0}.{1:6} seconds".format(pet_delay_diff /1000000000, pet_delay_diff % 1000000000))
