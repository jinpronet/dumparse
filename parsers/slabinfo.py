...
class Slabinfo(RamParser) :
    def get_free_pointer(self, ramdump, s, obj) :
        ...
        slab_offset_offset = self.ramdump.field_offset("struct kmem_cache", "offset")
    ...
    def slab_index(self, ramdump, p, addr, slab) :
        slab_size_offset = self.ramdump.field_offset("struct kmem_cache", "size")
    ...
    def get_track(self, ramdump, slab, obj, track_type) :
        ...
        slab_offset_offset = self.ramdump.field_offset("struct kmem_cache", "offset")
        slab_inuse_offset = self.ramdump.field_offset("struct kmem_cache", "inuse")
    ...
    def get_nobjects(self, ramdump, page) :
        if re.search('3\.0\.\d',self.ramdump.version) is not None :
            n_objects_offset = self.ramdump.field_offset("struct page", "objects")
            ...
            map_count_offset = self.ramdump.field_offset("struct page", "_mapcount")
    ...
    def parse(self) :
        slab_out = open("{0}/slabs.txt".format(self.ramdump.outdir),"wb")
        ...
        slab_list_offset = self.ramdump.field_offset("struct kmem_cache", "list")
        slab_name_offset = self.ramdump.field_offset("struct kmem_cache", "name")
        slab_node_offset = self.ramdump.field_offset("struct kmem_cache", "node")
        cpu_cache_page_offset = self.ramdump.field_offset("struct kmem_cache_cpu", "page")
        cpu_slab_offset = self.ramdump.field_offset("struct kmem_cache", "cpu_slab")
        slab_partial_offset = self.ramdump.field_offset("struct kmem_cache_node", "partial")
        slab_full_offset = self.ramdump.field_offset("struct kmem_cache_node", "full")
        ...
        while slab != original_slab :
            ...
            self.print_slab_page_info(self.ramdump, slab, slab_node, slab_node_addr + slab_partial_offset, slab_out)
            self.print_slab_page_info(self.ramdump, slab, slab_node, slab_node_addr + slab_full_offset, slab_out)

            for i in range(0,cpus) :
                cpu_slabn_addr = cpu_slab_addr + self.ramdump.read_word(per_cpu_offset + 4*i)
                self.print_per_cpu_slab_info(self.ramdump, slab, slab_node, cpu_slabn_addr + cpu_cache_page_offset, slab_out)

            slab = self.ramdump.read_word(slab + slab_list_offset)
        print_out_str("---wrote slab information to {0}/slabs.txt".format(self.ramdump.outdir))
