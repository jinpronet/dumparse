...
class Workqueues(RamParser):
    ...
    def print_workqueue_state_3_0(self, ram_dump) :
        ...
        idle_list_offset = ram_dump.field_offset("struct global_cwq", "idle_list")
        worklist_offset = ram_dump.field_offset("struct global_cwq", "worklist")
        busy_hash_offset = ram_dump.field_offset("struct global_cwq", "busy_hash")
        ...
        work_entry_offset = ram_dump.field_offset("struct work_struct", "entry")
        ...
        current_work_offset = ram_dump.field_offset("struct worker", "current_work")
        cpu_wq_offset = ram_dump.field_offset("struct workqueue_struct", "cpu_wq")
    ...
    def print_workqueue_state_3_7(self, ram_dump) :
        ...
        current_work_offset = ram_dump.field_offset("struct worker", "current_work")
        cpu_wq_offset = ram_dump.field_offset("struct workqueue_struct", "cpu_wq")
        pool_idle_offset = ram_dump.field_offset("struct worker_pool", "idle_list")
        worker_pool_size = ram_dump.sizeof("struct worker_pool")
        pending_work_offset = ram_dump.field_offset("struct worker_pool", "worklist")
        unbound_gcwq_addr = ram_dump.addr_lookup("unbound_global_cwq")
    ...
    def print_workqueue_state_3_10(self, ram_dump) :
        ...
        busy_hash_offset = ram_dump.field_offset("struct worker_pool", "busy_hash")
        scheduled_offset = ram_dump.field_offset("struct worker", "scheduled")
        worker_task_offset = ram_dump.field_offset("struct worker", "task")
        worker_entry_offset = ram_dump.field_offset("struct worker", "entry")
        offset_comm = ram_dump.field_offset("struct task_struct", "comm")
        ...
        for i in range(0,cpus) :
            ...
            for k in range(0, 2) :
                worker_pool_i = worker_pool + k * worker_pool_size
                busy_hashi = ram_dump.read_string(worker_pool_i + busy_hash_offset,s)
                for a in busy_hashi :
                    busy_hash.append(a)
    ...
    def parse(self) :
            ...
            if re.search('3.4.\d',ver) is not None :
                    ...
                    idle_list_offset = self.ramdump.field_offset("struct global_cwq", "idle_list")
