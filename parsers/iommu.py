...
class IOMMU(RamParser):
	...
	def __init__(self, *args):
		...
		self.node_offset = self.ramdump.field_offset('struct msm_iova_data', 'node')
		self.domain_num_offset = self.ramdump.field_offset('struct msm_iova_data', 'domain_num')
		self.domain_offset = self.ramdump.field_offset('struct msm_iova_data', 'domain')
		self.priv_offset = self.ramdump.field_offset('struct iommu_domain', 'priv')
		self.ctxdrvdata_attached_offset = self.ramdump.field_offset('struct msm_iommu_ctx_drvdata', 'attached_elm')
		self.ctxdrvdata_name_offset = self.ramdump.field_offset('struct msm_iommu_ctx_drvdata', 'name')
		self.ctxdrvdata_num_offset = self.ramdump.field_offset('struct msm_iommu_ctx_drvdata', 'num')
		self.priv_pt_offset = self.ramdump.field_offset('struct msm_iommu_priv', 'pt')
		self.list_attached_offset = self.ramdump.field_offset('struct msm_iommu_priv', 'list_attached')
		self.client_name_offset = self.ramdump.field_offset('struct msm_iommu_priv', 'client_name')
		self.pgtable_offset = self.ramdump.field_offset('struct msm_iommu_pt', 'fl_table')
		self.redirect_offset = self.ramdump.field_offset('struct msm_iommu_pt', 'redirect')

		self.list_next_offset, self.list_prev_offset = llist.get_list_offsets(self.ramdump)
	...
	def list_func(self, node):
		ctx_drvdata_name_ptr = self.ramdump.read_word(node + self.ctxdrvdata_name_offset)
	...
	def iommu_domain_func(self, node):
		...
		if self.client_name_offset is not None:
			client_name_ptr = self.ramdump.read_word(priv_ptr + self.client_name_offset)
		...
		if self.list_attached_offset is not None:
			list_attached = self.ramdump.read_word(priv_ptr + self.list_attached_offset)
		else:
			list_attached = None

		if self.priv_pt_offset is not None:
			pg_table = self.ramdump.read_word(priv_ptr + self.priv_pt_offset + self.pgtable_offset)
			redirect = self.ramdump.read_word(priv_ptr + self.priv_pt_offset + self.redirect_offset)
		else:
		...
		if list_attached is not None and list_attached != 0:
			list_walker = llist.ListWalker(self.ramdump, list_attached, self.ctxdrvdata_attached_offset, self.list_next_offset, self.list_prev_offset)
	...
	def parse(self):
		...
		iommu_domains_rb_root_addr = self.ramdump.read_word(iommu_domains_rb_root)
