...
class DebugImage(RamParser) :
    ...
    def parse_cpu_ctx (self, start, end, tag) :
        ...
        if magic is None :
            print_out_str("!!! Address {0:x} is bogus! Can't parse!".format(start))
            return
        ...
        for i in range(regs.ncores) :
            regs.dump_core_pc(i)
        regs.dump_all_regs(self.ramdump.outdir)
    ...
    def parse_l2_cache (self, start, end, tag) :
        ...
        parse_cache_dump(self.ramdump, self.ramdump.outdir, start)

    def parse_l1_cache (self, start, end, tag) :
        print_out_str ("Parsing L1 cache context start {0:x} end {1:x}".format(start, end))
        magic = self.ramdump.read_word(start, False)
        if magic is None :
            print_out_str("!!! Address {0:x} is bogus! Can't parse!".format(start))
            return
        ...
        save_l1_dump(self.ramdump, self.ramdump.outdir, start, end - start)
    ...
    def parse(self) :
        ...
        self.load_numbers()
        dump_table_ptr_offset = self.ramdump.field_offset("struct msm_memory_dump", "dump_table_ptr")
        version_offset = self.ramdump.field_offset("struct msm_dump_table", "version")
        num_entries_offset = self.ramdump.field_offset("struct msm_dump_table", "num_entries")
        client_entries_offset = self.ramdump.field_offset("struct msm_dump_table", "client_entries")
        id_offset = self.ramdump.field_offset("struct msm_client_dump", "id")
        start_addr_offset = self.ramdump.field_offset("struct msm_client_dump", "start_addr")
        end_addr_offset = self.ramdump.field_offset("struct msm_client_dump", "end_addr")
        ...
        for i in range(0,num_entries) :
            ...
            client_start = self.ramdump.read_word(this_client + start_addr_offset)
        ...
        self.qdss.dump_all(self.ramdump, out_dir)
