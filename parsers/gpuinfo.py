...
@register_parser("--print-gpuinfo", "print gpu info like ringbuffer,snapshot and pointer addresses")
class GPUinfo(RamParser):
    def parse(self) :
            ...
            #there is bug if field_offset is None ;the + not add int wiht None
            # I change a default value
            memptrs_desc_f = self.ramdump.field_offset(
                            "struct adreno_ringbuffer", "memptrs_desc") if self.ramdump.field_offset(
                            "struct adreno_ringbuffer", "memptrs_desc") is not None else 0
            ...
            current_context = self.ramdump.read_word(int(memstore_addr) + 32, False)
            retired_timestamp = self.ramdump.read_word(int(memstore_addr) + 8, False)
            i = 0
            for i in range(0, int(ringbuffer_size)) :
                    data = self.ramdump.read_word(int(ringbuffer_addr) + (i * 4), False)
