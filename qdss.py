...
class QDSSDump() :
    ...
    def print_tmc_etf (self, ram_dump, out_dir) :
        if self.tmc_etf_start is None :
            print_out_str ("!!! TMC-ETF address has not been set! I can't continue!")
            return

        print_out_str("Now printing TMC-ETF registers to file")
        tmc_etf_out = open("{0}/tmc_etf.txt".format(out_dir),"wb")
        for a,b in tmc_registers.iteritems() :
            offset, name = b
            tmc_etf_out.write("{0} ({1}) : {2:x}\n".format(a, name, ram_dump.read_word(self.tmc_etf_start + offset, False)))
        tmc_etf_out.close()

    def print_tmc_etr (self, ram_dump, out_dir) :
        if self.tmc_etr_start is None :
            print_out_str ("!!! TMC-ETR address has not been set! I can't continue!")
            return

        print_out_str("Now printing TMC-ETR registers to file")
        tmc_etf_out = open("{0}/tmc_etr.txt".format(out_dir),"wb")
        for a,b in tmc_registers.iteritems() :
            offset, name = b
            tmc_etf_out.write("{0} ({1}) : {2:x}\n".format(a, name, ram_dump.read_word(self.tmc_etr_start + offset, False)))
        tmc_etf_out.close()

    def print_etm_registers(self, ram_dump, out_dir, base, fname) :
        etm_out = open("{0}/{1}.txt".format(out_dir,fname),"wb")
        for a,b in etm_registers.iteritems() :
            offset, name = b
            etm_out.write("{0} ({1}) : {2:x})\n".format(a, name, ram_dump.read_word(base + offset * 4, False)))
        etm_out.close()

    def print_all_etm_register(self, ram_dump, out_dir) :
        if self.etm_regs0 is None:
            print_out_str ("!!! ETM REGS 0 address was not set! Nothing will be parsed")
        else :
            self.print_etm_registers(ram_dump, out_dir, self.etm_regs0, "etm_regs0")

        if self.etm_regs1 is None:
            print_out_str ("!!! ETM REGS 1 address was not set! Nothing will be parsed")
        else :
            self.print_etm_registers(ram_dump, out_dir, self.etm_regs1, "etm_regs1")

        if self.etm_regs2 is None:
            print_out_str ("!!! ETM REGS 2 address was not set! Nothing will be parsed")
        else :
            self.print_etm_registers(ram_dump, out_dir, self.etm_regs2, "etm_regs2")

        if self.etm_regs3 is None:
            print_out_str ("!!! ETM REGS 3 address was not set! Nothing will be parsed")
        else :
            self.print_etm_registers(ram_dump, out_dir, self.etm_regs3, "etm_regs3")


    def save_etf_bin(self, ram_dump, out_dir) :
         tmc_etf = open("{0}/tmc-etf.bin".format(out_dir),"wb")
    ...
    def save_etr_bin(self, ram_dump, out_dir) :
        tmc_etr = open("{0}/tmc-etr.bin".format(out_dir),"wb")
        if self.tmc_etr_start is None :
        ...
        if (ctl & 0x1) == 1 and (mode == 0 ) :
            ...
            dbalo = ram_dump.read_word(self.tmc_etr_start + dbalo_offset, False)
    ...
    def dump_all(self, ram_dump, out_dir) :
        self.print_tmc_etf(ram_dump, out_dir)
        self.print_tmc_etr(ram_dump, out_dir)
        self.print_all_etm_register(ram_dump, out_dir)
        self.save_etf_bin(ram_dump, out_dir)
        self.save_etr_bin(ram_dump, out_dir)
